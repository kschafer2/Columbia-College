...
//
//  Created by Kyle Schafer on 7/10/17.
...
int main()
{
    outFile.open("trans.out", ios::out); //opens "trans.out" for output
...
void displayMenu(Operation &choice)
{
    ...
    again = true; //set again to true for next check
...
{
    cout << endl
    << "1. PED - Pediatrics                  7.  PSY - Psychiatry" << endl
    << "2. GEN - General practice            8.  NEU - Neurology" << endl
    << "3. INT - Internal medicine           9.  ORT - Orthopedics" << endl
    << "4. CAR - Cardiology                  10. DET - Dermatology" << endl
    << "5. SUR - Surgeon                     11. OPT - Ophthalmology" << endl
    << "6. OBS - Obstetrics                  12. ENT - Ear, Nose, and Throat" << endl << endl;
...
/*--------------------------------------------------------------------------------------------
 3. FUNCTION ASSIGN DOCTOR
 ---------------------------------------------------------------------------------------------
 
 Assigns doctor to preferred room if available
 If doctor has same name as another doctor already in the clinic, request is denied
 If room is already occupied by another doctor, request is denied
...
bool assign(Doctor &newDoctor)
{
    ...
    //looks through clinic for doctor with same name
    //if a doctor with the same name is found, request is denied
    //returns false
    ...
    //if the preferred room is already being used, request is denied
    //returns false
...
/*--------------------------------------------------------------------------------------------
 4. FUNCTION ASSIGN PATIENT
 ---------------------------------------------------------------------------------------------
 
 -Attempts to assign patient to a doctor with specialty code supplied in argument 2
 -Returns true if patient is assigned
 -Returns false if patient is not assigned
...
/*--------------------------------------------------------------------------------------------
 5. FUNCTION ASSIGN PATIENT BEST FIT
 ---------------------------------------------------------------------------------------------
 -If patient is under 16 (child), patient is assigned to a pediatrician (PED)
 -Otherwise, assigns patient to room with doctor of specified code
 -If there is a patient in the room, patient is assigned to least crowded waiting room of room
 with doctor of specified code
 -If no doctor of specified code, patient assigned to GP
 -If there is a patient in the room, patient is assigned to least crowded waiting room of room
 with GP
 -If no GP, patient is assigned to a random specialty code, not including GP or PED (must save 
 PED for those under 16, as they can only be seen by a PED), then assigned to an appropriate
 exam room or waiting room
...
void assignBestFit(Patient &newPatient)
{
    ...
    //if patient is still unassigned, tries to assign patient to another doctor starting with INT
    //(PED = 1 and GEN = 2 are already ruled out)
    //INT is code #3, continues until code #12 (ENT)
    if(!assigned && code != PED)
    {
...
/*--------------------------------------------------------------------------------------------
 6. FUNCTION DOCTOR CHECK-IN
 ---------------------------------------------------------------------------------------------
 
-Checks a doctor into preferred room if it's available
-If preferred room is unavailable or another doctor by the same name is in the clinic,
check-in request is denied.
...
void doctorCheckIn()
{
    ...
    again = true; //reset again for specialty loop
...
/*--------------------------------------------------------------------------------------------
 7. FUNCTION DOCTOR CHECK-OUT
 ---------------------------------------------------------------------------------------------
 
 Checks a doctor out of the clinic.
 Searches for a doctor with given name.
...
void doctorCheckOut()
{
    ...
    for(int i = 0; i < NUM_CLINIC_ROOMS; i++)
    {
        ...
        if(rooms[i].hasDoctor())
        {
            ...
            //if the names match, set local room to current room
            //room is set to index number + 1 for output formatting purposes
            //found set to true
...
/*--------------------------------------------------------------------------------------------
 8. FUNCTION PATIENT CHECK-IN
 ---------------------------------------------------------------------------------------------
 
 -Checks-in a patient if qualified doctor is in the clinic
 -Assigns patient to an empty exam room or to a waiting room
...
void patientCheckIn()
{
    ...
    else
    {
        ...
        again = true; //again reset to true for specialty code loop
...
/*--------------------------------------------------------------------------------------------
 9. FUNCTION PATIENT CHECK-OUT
 ---------------------------------------------------------------------------------------------
 
 -Looks for patient by name
 -If patient is found, patient is removed from clinic, and the new exam room patient is the 
 patient at the front of the waiting room queue
...
void patientCheckOut()
{
    ...
    for(int i = 0; i < NUM_CLINIC_ROOMS; i++)
    {
        if(rooms[i].hasPatient())
        {
            ...
            //if the patient's name is found, found becomes true and breaks out of loop
            //local room holds address of room with found patient
            //local patient holds info of found room patient
...
/*--------------------------------------------------------------------------------------------
 ...
 -Displays farewell message, closes output file
...
/*--------------------------------------------------------------------------------------------
 11. FUNCTION ANOTHER USER?
 ---------------------------------------------------------------------------------------------
 
 -Asks if there's another user
 -Returns true if Y
 -Returns false if N
...
/*--------------------------------------------------------------------------------------------
 12. FUNCTION ASSIGN GENERATED DOCTOR
 ---------------------------------------------------------------------------------------------
 
 -Assigns generated doctor to specified room #
 -Random assignment function currently doesn't assign room number randomly
...
/*--------------------------------------------------------------------------------------------
 13. FUNCTION GENERATE CLINIC
 ---------------------------------------------------------------------------------------------
 
 -Generates a number of doctors and patients
 -Assigns doctors in order based on index number
 -Randomly assigns doctor specialty code
 -Sets patient specialty code randomly if patient is at least 16
 
 */

void GenerateClinic()
{
    int NUM_DOCS = (rand() % 100) + 1; //assigns between 1 and 100 doctors
    int NUM_PATS = (rand() % 500) + 1; // assigns between 1 and 500 patients
    
    Doctor testDoc[NUM_DOCS]; //array of doctors of number NUM_DOCS
    Patient testPat[NUM_PATS]; //array of patients of number NUM_PATS
    
    //assigns each doctor an index-based name and room number
    //assigns each doctor a specialty code
    ...
    //for each generated patient, age set between 1 and 100
    //code set according to age
    //name set in same fashion as doctors
